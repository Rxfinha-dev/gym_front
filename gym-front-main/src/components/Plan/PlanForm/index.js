...
const PlanRegistration = () => {
  ...
  const handlePlanRegister = async (e) => {
    e.preventDefault();
    try {
      setLoading(true);
      const numericPrice = parseFloat(price.replace(/[R$\s.]/g, '').replace(',', '.'));
      await apiPost('/plan', { name, price: numericPrice });
      setErrors([]);
      navigate('/plan-list');
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <Paper elevation={3} sx={{ padding: 3, borderRadius: 3, maxWidth: 800, width: '100%', boxShadow: '0px 4px 20px rgba(0, 0, 0, 0.1)' }}>
        <Grid
          container
          alignItems="center"
          spacing={2}
        ...
        <form onSubmit={handlePlanRegister}>
